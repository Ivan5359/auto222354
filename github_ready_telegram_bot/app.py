...
def generate_accounts(limit=10):
        ...
        result.append(
            {
                ...
                "niche": niche,
                "followers": random.randint(300, 50000),
                "has_site": random.choice([0, 1]),
            }
        )
    return result
...
def ensure_accounts():
    ...
    try:
        ...
        if count == 0:
            for account in generate_accounts(20):
                conn.execute(
                    """
...
def accounts_text():
    ...
    for i, row in enumerate(rows, 1):
        ...
        lines.append(
            f"{i}. @{escape(row['username'])} | {escape(row['niche'])} | "
            f"{row['followers']} подписчиков | {site} | {escape(row['status'])}"
        )
    lines.append("")
    lines.append("Это безопасные тестовые аккаунты, не реальные Instagram-профили.")
    return "\n".join(lines)
...
def status_text():
    conn = db()
    try:
        rows = conn.execute("SELECT status, COUNT(*) AS c FROM accounts GROUP BY status").fetchall()
    finally:
        conn.close()
...
def help_text():
    return (
        ...
        "1. Нажми <b>Аккаунты</b> - бот покажет тестовые аккаунты.\n"
        "2. Нажми <b>Запустить</b> - начнется безопасный dry-run.\n"
        "3. Нажми <b>Статистика</b> - увидишь прогресс.\n"
        "4. Нажми <b>Стоп</b> - тест остановится.\n\n"
...
def run_test():
    ...
    try:
        rows = conn.execute(
            "SELECT username FROM accounts WHERE status = 'new' LIMIT 20"
        ).fetchall()
        done = 0
        for row in rows:
            if state["stop"]:
...
def setup():
    ...
    response = tg("setWebhook", {"url": webhook_url})
